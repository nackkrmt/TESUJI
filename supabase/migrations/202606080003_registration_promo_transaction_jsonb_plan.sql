...
begin
  ...
  loop
    ...
    v_plan := v_plan || jsonb_build_array(
      jsonb_build_object(
        'sortOrder', array_position(p_division_ids, v_division.id),
        'divisionId', v_division.id,
        'divisionName', v_division.name,
  ...
  select coalesce(sum(plan."feeAmount"), 0)
    into v_total_fee_amount
    from jsonb_to_recordset(v_plan) as plan(
      "feeAmount" numeric,
      status text
    )
  ...
  if v_promo_code_input is not null then
    ...
    select coalesce(sum(plan."feeAmount"), 0), count(*)
    ...
    if v_promo.division_ids is not null and exists (
      ...
      where plan.status <> 'waiting_list'
        and plan."feeAmount" > 0
        and not (plan."divisionId" = any(v_promo.division_ids))
    ) then
    ...
    for v_plan_row in
      ...
      order by plan."sortOrder"
    loop
      v_row_discount := 0;

      if v_plan_row.status <> 'waiting_list' and v_plan_row."feeAmount" > 0 then
        v_discount_index := v_discount_index + 1;

        if v_discount_index = v_discount_rows then
          v_row_discount := least(v_plan_row."feeAmount", greatest(v_remaining_discount, 0));
        elsif v_promo.discount_type = 'free' then
          v_row_discount := v_plan_row."feeAmount";
        elsif v_promo.discount_type = 'percentage' then
          v_row_discount := least(
            v_plan_row."feeAmount",
            round(v_plan_row."feeAmount" * v_promo.discount_value / 100, 2)
          );
        else
          v_row_discount := least(v_plan_row."feeAmount", v_remaining_discount);
        end if;

        v_remaining_discount := v_remaining_discount - v_row_discount;
      end if;

      v_discounted_plan := v_discounted_plan || jsonb_build_array(
        jsonb_build_object(
          'sortOrder', v_plan_row."sortOrder",
          'divisionId', v_plan_row."divisionId",
          'divisionName', v_plan_row."divisionName",
          'feeAmount', v_plan_row."feeAmount",
          'discountAmount', v_row_discount,
          'status', v_plan_row.status,
          'waitingListPosition', v_plan_row."waitingListPosition"
        )
      );
    end loop;

    v_plan := v_discounted_plan;

    select count(*), coalesce(sum(plan."discountAmount"), 0)
      into v_usage_increment, v_discount_amount
      from jsonb_to_recordset(v_plan) as plan("discountAmount" numeric)
      where plan."discountAmount" > 0;
  ...
  select coalesce(sum(plan."discountAmount"), 0)
  ...
  for v_plan_row in
    ...
    order by plan."sortOrder"
  loop
    if v_plan_row.status = 'waiting_list' then
      v_registration_status := 'waiting_list';
    elsif v_plan_row."feeAmount" - v_plan_row."discountAmount" = 0 then
      v_registration_status := 'confirmed';
    else
      v_registration_status := 'pending_payment';
    end if;
    ...
    values (
      ...
      v_source,
      v_plan_row."feeAmount",
      v_plan_row."discountAmount",
      v_plan_row."waitingListPosition",
      case when v_registration_status = 'confirmed' then v_now else null end
    )
    returning id into v_registration_id;

    if v_has_promo and v_plan_row."discountAmount" > 0 then
    ...
    v_registration_results := v_registration_results || jsonb_build_array(
      jsonb_build_object(
        ...
        'status', v_registration_status,
        'feeAmount', v_plan_row."feeAmount",
        'discountAmount', v_plan_row."discountAmount",
        'finalFeeAmount', v_plan_row."feeAmount" - v_plan_row."discountAmount",
        'waitingListPosition', v_plan_row."waitingListPosition"
      )
    );
  end loop;
